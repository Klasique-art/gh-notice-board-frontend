...
interface OpportunityDetailShareProps {
    opportunity: Opportunity;
}

const OpportunityDetailShare = ({ opportunity }: OpportunityDetailShareProps) => {
    const [copied, setCopied] = useState(false);

    // Get current URL
    const opportunityUrl = typeof window !== "undefined" ? window.location.href : "";

    // Share handlers
    const shareOnFacebook = () => {
        window.open(
            `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(opportunityUrl)}`,
            "_blank",
    ...
    const shareOnTwitter = () => {
        window.open(
            `https://twitter.com/intent/tweet?url=${encodeURIComponent(opportunityUrl)}&text=${encodeURIComponent(opportunity.title)}`,
            "_blank",
            "width=600,height=400"
        );
    };
    ...
    const shareOnLinkedIn = () => {
        window.open(
            `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(opportunityUrl)}`,
            "_blank",
            "width=600,height=400"
        );
    };
    ...
    const shareViaEmail = () => {
        window.location.href = `mailto:?subject=${encodeURIComponent(opportunity.title)}&body=${encodeURIComponent(`Check out this opportunity: ${opportunity.title}\n\n${opportunityUrl}`)}`;
    };
    ...
    return (
        ...
        >
            <div className="space-y-4">
                <h3 className="big-text-5 font-bold text-slate-900">Share this opportunity</h3>
...
export default OpportunityDetailShare;